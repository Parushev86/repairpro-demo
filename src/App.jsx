      ...
      <main style={{flex:1, overflow:"auto", display:"flex", flexDirection:"column"}}>
        ...
        {tab==="technicians"  && isAdmin && <TechniciansTab technicians={technicians} orders={orders} onSave={saveTech} onDelete={handleDeleteTech} onExport={()=>exportTechReport(technicians,orders)}/>}
          ...
          {tab==="phonesales"   && <PhoneSalesTab
            ...
            onSave={async r=>{
              try {
                // Премахни _inv_id от данните преди запис в Supabase
                ...
                if(!r.id) {
                  setPhoneSales(p=>[s,...p]);
                  // Намали наличността в склада ако е избран от инвентара
                  if(_inv_id) {
...
function PricingTab() {
  ...
  useEffect(() => {
    const loadPrices = async () => {
      try {
        const sb = getSupabase();
        if(!sb) { setPrices(JSON.parse(localStorage.getItem("rp_prices") || "null") || DEFAULT_PRICES); return; }
        const { data } = await sb.from("pricing").select("*").eq("key","prices").single();
        if(data?.data) setPrices(data.data);
        else setPrices(JSON.parse(localStorage.getItem("rp_prices") || "null") || DEFAULT_PRICES);
      } catch(e) {
        setPrices(JSON.parse(localStorage.getItem("rp_prices") || "null") || DEFAULT_PRICES);
  ...
  const saveCustomPrices = async (data) => {
    ...
    try {
      const sb = getSupabase();
      if(sb) {
        await sb.from("pricing").upsert({key:"prices", data}, {onConflict:"key"});
      }
    } catch(e) { console.warn("Pricing save error:", e.message); }
    setEditMode(false);
    setEditData(null);
    setNewModelName("");
  };
  ...
  const service = prices[activeService] || Object.values(prices)[0];
  // Fix activeService if current key doesn't exist in prices
  useEffect(() => {
    if(pricesLoaded && prices && !prices[activeService]) {
      const firstKey = Object.keys(prices)[0];
      if(firstKey) setActiveService(firstKey);
    }
  }, [pricesLoaded, prices]);
