...
export default function App() {
 ...
 const filteredOrders = orders.filter(o => {
  ...
  expenses={[...expenses, ...monthlyExpenses.filter(e => e.date)]} cashRegister={cashReg} isAdmin={true}
...
/>}
          {tab === "expenses" && !isAdmin && <ExpensesTab
  expenses={[...expenses, ...monthlyExpenses.filter(e => e.date)].filter(e => e.date === today())} cashRegister={cashReg} isAdmin={false}
  orders={orders} accSales={accSales} partsSales={partsSales} phoneSales={phoneSales}
  onSaveExpense={async r => {
    const s = await upsertExpense(r);
    if (!r.id) setExpenses(p => [s, ...p]);
